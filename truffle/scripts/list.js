...
const NFT_CONTRACT = "0xc3696BcdE967a4fD1C9236F0Bb5F5E1d31DeA501";
const TOKEN_ID = 1;
const PRICE = 1;
const START = TODAY;
const END = TOMORROW;
...
const main = async (cb) => {
  try {
    ...
    let txn = await nftContract.methods
      .approve(marketplace.address, TOKEN_ID)
      .send({ from: owner });
    console.log(txn);
    txn = await marketplace.listNft(NFT_CONTRACT, TOKEN_ID, PRICE, START, END, {
      from: owner,
      value: listingFee,
    });
    console.log(txn);
