...
contract("Marketplace", function (accounts) {
  ...
  it("should revert if conditions are not satisfied for listing nft", async () => {
    expectRevert(
      marketplace.listNft(rentableNft.address, tokenId1, 1, TODAY, YESTERDAY, {
        from: USER,
        value: listingFee,
      }),
    ...
    expectRevert(
      marketplace.listNft(rentableNft.address, tokenId1, 1, TODAY, TOMORROW, {
        from: TOKEN_OWNER,
        value: listingFee - 1,
      }),
    ...
    expectRevert(
      marketplace.listNft(rentableNft.address, tokenId1, 1, YESTERDAY, TODAY, {
        from: TOKEN_OWNER,
        value: listingFee,
      }),
